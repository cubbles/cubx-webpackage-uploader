...
Uploader.prototype.uploadSingleWebpackage = function (passedConfig, done) {
  ...
  if (passedConfig.target.proxy) process.env.http_proxy = passedConfig.target.proxy;

  // Request an access_token ... and run the upload.
  cubxAuthenticationClient(this.providedConfigObject.nano.url, this.providedConfigObject.store, user, password, function (err, access_token) {
    if (err) {
      done(err);
    } else {
      this.providedConfigObject.nano.cookie = 'access_token=' + access_token;
...
Uploader.prototype._insertOrUpdateSingleWebpackage = function (store, config, done) {
  ...
  function doUpload (db, doc, config, done) {
      ...
      db.atomic('couchapp-webpackage-validator', 'startUpload', doc._id, doc, function (err, body) {
        if (err) {
          ...
          db.get(doc._id, { revs_info: false }, function (err, body) {
            ...
            uploaderInstance._collectFiles(config.sourcePath, function (filesForUpload, fileToBeIgnored) {
              if (uploaderInstance.providedConfigObject.dryRun) {
                var dryResponse = {
                  id: body._id,
                  dryRun: true,
                  filesForUpload: [],
                  filesIgnored: fileToBeIgnored
                };
                filesForUpload.forEach(function (absoluteFileName) {
                  dryResponse.filesForUpload.push(absoluteFileName.replace(config.sourcePath + path.sep, '').replace('\\', '/'));
                });
  ...
  webpackageDocJsonPromise.then(function (webpackageDocumentJson) {
    ...
    return new Promise(
      function (resolve, reject) {
        ...
        doUpload(store, webpackageDocumentJson, config, uploadCallback);
      }
    );
...
/**
 * Upload a webpackage into a base.
 * @param {Object} db  nano db object
 * @param {Object} doc webpackageDocument
 * @param {Object} config config object
 * @param {function} done to be called, if upload finished.
 */
// Uploader.prototype._doUpload = function (db, doc, config, done) {
//   db.get(doc._id, { revs_info: false }, function (err, body) {
//     if (!err && body._rev) {
//       doc._rev = body._rev;
//     }
...
//             return;
//           }
//           this._collectFiles(config.sourcePath, function (filesForUpload) {
//             if (this.providedConfigObject.dryRun) {
//               this.filesForUpload = filesForUpload;
//               done(undefined, { id: body._id });
//             } else {
//               this._insertEachFileAsAttachment(filesForUpload, config.sourcePath, db, body._id, body._rev, function (err) {
//                 if (err) {
//                   done(err);
//                   return;
//                 }
 ...
 * @param {callback} a function to process the files collected
 */
    ...
    walk.walkSync(webpackageRoot, {
      listeners: {
        names: function (root, nodeNamesArray) {
          ...
          var relativePosixFilename = absoluteFileName.replace(webpackageRoot + path.sep, '').replace('\\', '/');
          if (ignoreConfiguration.indexOf(relativePosixFilename) === -1) {
            filesToBeUploaded.push(absoluteFileName);
          } else {
            filesToBeIgnored.push(relativePosixFilename);
          }
          next();
