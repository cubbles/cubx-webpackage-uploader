...
function Uploader () {
  this.resolvedConfigObject = undefined;
}
...
Uploader.prototype.uploadSingleWebpackage = function (passedConfig, done) {
  ...
  if (configProvider.getProvidedConfigObject().target.proxy) {
    if (this.resolvedConfigObject.nano.url.indexOf('http:') === 0) {
      process.env.http_proxy = passedConfig.target.proxy;
    }
    if (this.resolvedConfigObject.nano.url.indexOf('https:') === 0) {
      process.env.https_proxy = passedConfig.target.proxy;
    }
  }

  // Request an access_token ... and run the upload.
  cubxAuthenticationClient(this.resolvedConfigObject.nano.url, this.resolvedConfigObject.store, user, password, function (err, accessToken) {
    if (err) {
      done(err);
    } else {
      this.resolvedConfigObject.nano.cookie = 'access_token=' + accessToken;
      this._useStore(this.resolvedConfigObject, function (store) {
        this._insertOrUpdateSingleWebpackage(store, this.resolvedConfigObject, done);
      }.bind(this), done);
    }
  }.bind(this));
};
  ...
  webpackageDocJsonPromise.then(function (webpackageDocumentJson) {
    ...
    return new Promise(
      function (resolve, reject) {
        ...
        if (uploaderInstance.resolvedConfigObject.dryRun) {
  ...
  var addDocumentId = function (manifest) {
    ...
    var onValidationError = function (errors) {
      ...
      errors.forEach(function (error) {
        ...
        errorString += (error.dataPath) ? error.dataPath + ': ' + message : message;
      });
...
Uploader.prototype._debug = function (payload) {
  if (this.resolvedConfigObject && this.resolvedConfigObject.debug && this.resolvedConfigObject.debug === true) {
