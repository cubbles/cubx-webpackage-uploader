...
Uploader.prototype._collectFiles = function (webpackageRoot, processCollectedFiles) {
  ...
  glob(globsArray, { cwd: webpackageRoot, root: webpackageRoot }, function (err, arrayOfFilesToBeIgnored) {
    ...
    function isInExcludedDir (absolutePath) {
      var exclude = false;
      ignoreConfiguration.forEach((conf) => {
        if (conf.endsWith('/')) {
          var strippedConf = conf.substr(0, conf.length - 1);
          var regexStr = '.*' + strippedConf + '\\' + path.sep + '[^' + '\\' + path.sep + ']+$';
    ...
    walk.walkSync(webpackageRoot, {
      listeners: {
        ...
        file: function (root, fileStats, next) {
          ...
          if (ignoreConfiguration.indexOf(relativePosixFilename) === -1 && !isInExcludedDir(absoluteFileName)) {
